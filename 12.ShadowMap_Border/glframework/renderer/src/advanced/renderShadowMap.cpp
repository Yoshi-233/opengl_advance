                               ...
                               const std::shared_ptr<Framebuffer> &fbo)
{
        ...
        auto lightMatrix = Renderer::getLightMatrix(directionalLight);
...
glm::mat4 Renderer::getLightMatrix(const std::shared_ptr<DirectionalLight> &directionalLight)
{
        /* 1. viewMatrix */
        auto lightViewMatrix = glm::inverse(directionalLight->getModelMatrix());

        /* 2. projectionMatrix(正交投影) */
        float size = 6.0f;
        auto lightCamera = std::make_shared<OrthographicCamera>(-size, size,
                                                                -size, size,
                                                                0.1f, 80.0f);
