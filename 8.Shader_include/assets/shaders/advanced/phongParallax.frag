...
vec2 parallaxUV(vec2 uv, vec3 viewDir)
{
        ...
        vec2 deltaUV = viewDir.xy / viewDir.z * heightScale / layerNum; // 每一次的最大偏移量

        /* 2.initialize */
        vec2 currentUV = uv;
        float currentSampleDepth = texture(parallaxMapSampler, currentUV).r;
        float currentLayerDepth = 0.0f;

        /* 3.计算视差贴图的uv坐标 */
        while (currentLayerDepth < currentSampleDepth) {
                currentUV -= deltaUV;
                currentSampleDepth = texture(parallaxMapSampler, currentUV).r;
                currentLayerDepth += layerDepth;
        }

        /* 4.计算last legth */
        vec2 lastUV = currentUV + deltaUV;
        float lastLayerDepth = currentSampleDepth - layerDepth;
        float lastHeight = texture(parallaxMapSampler, lastUV).r;
        float lastLength = lastHeight - lastLayerDepth;

        /* 5.计算curlength */
        float currentLength = currentLayerDepth - currentSampleDepth;

        /* 6.计算权重 */
        float lastWeight = currentLength / (currentLength + lastLength);
...
void main()
{
        ...
        // 计算光照的通用数据
        vec3 objColor = texture(sampler, uvReal).rgb;//物体颜色
        float objAlpha = texture(sampler, uvReal).a;
        // diffuse 数据
        // vec3 normalN = normalize(normal);
        vec3 normalN = texture(normalMapSampler, uvReal).rgb * 2.0 - vec3(1.0); // 法线数据是从-1到1的，需要转换到-1-1
        // 经过TBN矩阵变换后的法线数据
