...
void Renderer::phongParallaxMaterialRender(const std::shared_ptr<Shader> &shaderPtr,
...
{
        auto phongMaterial = (PhongParallaxMaterial *) material.get();
        // diffuse贴图
        ...
        /* 高度/视差贴图 */
        shaderPtr->setInt("parallaxMapSampler", 3);
        phongMaterial->getParallaxMap()->bind();
        shaderPtr->setFloat("heightScale", phongMaterial->getHeightScale());
        shaderPtr->setFloat("layerNum", phongMaterial->getLayerNum());

        /* 透明度 */
        shaderPtr->setFloat("opacity", phongMaterial->mOpacity);

        Renderer::processSpecularMask<PhongParallaxMaterial>(shaderPtr, phongMaterial);
