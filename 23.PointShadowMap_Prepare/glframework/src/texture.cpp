...
Texture::Texture(const std::string &path, unsigned int unit, unsigned int internalFormat)
{
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, internalFormat, this->mWidth, this->mHeight,
...
Texture::Texture(unsigned int unit, unsigned char *dataIn, int width, int height, unsigned int internalFormat)
{
        this->mUnit = unit;
        int channels;
        /* 1. 读取图片时要反转y轴 */
...
Texture::Texture(unsigned int unit, int width, int height, unsigned int internalFormat)
{
        this->mUnit = unit;
        this->mWidth = width;
        this->mHeight = height;
...
std::shared_ptr<Texture> Texture::createDepthAttachment(unsigned int unit, int width, int height)
{
        ...
        GLfloat borderColor[] = {1.0f, 1.0f, 1.0f, 1.0f}; // 因为用的深度，这里代表深度为1.0，最大深度值
...
std::shared_ptr<Texture> Texture::createDepthAttachmentCSMArray(unsigned int unit, int width,
                                                                int height, unsigned int layerNum)
{
        std::shared_ptr<Texture> dTexture = std::make_shared<Texture>();
