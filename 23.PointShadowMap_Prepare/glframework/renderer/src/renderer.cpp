...
Renderer::Renderer()
{
        ...
        vertexPath = std::string(PROJECT_DIR) + "/assets/shaders/advanced/phongCSMShadow.vert";
        fragmentPath = std::string(PROJECT_DIR) + "/assets/shaders/advanced/phongCSMShadow.frag";
        this->mPhongCSMShadowShader = std::make_shared<Shader>(vertexPath.c_str(),
                                                               fragmentPath.c_str());

        vertexPath = std::string(PROJECT_DIR) + "/assets/shaders/advanced/shadowDistance.vert";
        fragmentPath = std::string(PROJECT_DIR) + "/assets/shaders/advanced/shadowDistance.frag";
        this->mShadowDistanceShader = std::make_shared<Shader>(vertexPath.c_str(),
                                                               fragmentPath.c_str());

        vertexPath = std::string(PROJECT_DIR) + "/assets/shaders/advanced/phongPointShadow.vert";
        fragmentPath = std::string(PROJECT_DIR) + "/assets/shaders/advanced/phongPointShadow.frag";
        this->mPhongPointShadowShader = std::make_shared<Shader>(vertexPath.c_str(),
                                                                 fragmentPath.c_str());
}
...
std::shared_ptr<Shader> Renderer::pickShader(MaterialType type)
{
        ...
        switch (type) {
                ...
                case MaterialType::PhongCSMShadowMaterial:
                        result = this->mPhongCSMShadowShader;
                        break;
                case MaterialType::PhongPointShadowMaterial:
                        result = this->mPhongPointShadowShader;
                        break;
                default:
...
)
{
        ...
        /* 渲染directional shadowmap */
        if(directionalLight != nullptr) {
                if(directionalLight->mUseCSM) {
                        this->renderDirectionalCSMShadowMap(camera, this->mOpacityObjects, directionalLight);
                } else {
                        this->renderDirectionalShadowMap(this->mOpacityObjects, directionalLight,
            ...
            object->getType() == ObjectType::INSTANCED_MESH) {
                ...
                switch (material->mType) {
                        ...
                        case MaterialType::PhongShadowMaterial: {
                                ...
                                break;
                        case MaterialType::PhongCSMShadowMaterial: {
                                Renderer::phongCSMShadowMaterialRender(shaderPtr, material, camera, mesh,
                                                                       directionalLight, pointLights,
                                                                       spotLight, ambientLight);
                        }
                                break;
                        case MaterialType::PhongPointShadowMaterial: {
                                Renderer::phongPointShadowMaterialRender(shaderPtr, material, camera, mesh,
                                                                         directionalLight, pointLights,
                                                                         spotLight, ambientLight);
                        }
                                break;
